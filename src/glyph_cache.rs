//! Glyph cache: builds curve and band textures for multiple glyphs.

use crate::font::BandData;
use std::collections::HashMap;

const BAND_TEXTURE_WIDTH: u32 = 4096;

/// Info for rendering a cached glyph.
...
/// Builds combined curve and band textures, and vertex data for text.
...
impl GlyphCache {
    ...
    /// Add a glyph's band data. Returns GlyphInfo for vertex generation.
    pub fn add_glyph(&mut self, glyph_id: u32, band_data: BandData) -> GlyphInfo {
        ...
        let band_start = self.alloc_bands(
            &band_data.band_texels,
            header_count,
            curve_start,
        );
    ...
    pub fn get_glyph(&self, glyph_id: u32) -> Option<GlyphInfo> {
        self.glyphs.get(&glyph_id).copied()
    }

    pub fn curve_data(&self) -> &[[f32; 4]] {
        &self.curve_texels
    }

    pub fn band_data(&self) -> &[[u32; 4]] {
        &self.band_texels
    }
