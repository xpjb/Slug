...
const kLogBandTextureWidth = 12u;
...
fn slug_render(
    curve_data: texture_2d<f32>,
    band_data: texture_2d<u32>,
    render_coord: vec2f,
    ...
    let hband_data = textureLoad(band_data, vec2i(glyph_loc.x + band_index.y, glyph_loc.y), 0).xy;
    var hband_loc = calc_band_loc(glyph_loc, hband_data.y);

    for (var curve_index: i32 = 0; curve_index < i32(hband_data.x); curve_index += 1) {
        let curve_loc = vec2i(
            i32(textureLoad(band_data, vec2i(hband_loc.x + curve_index, hband_loc.y), 0).x),
            i32(textureLoad(band_data, vec2i(hband_loc.x + curve_index, hband_loc.y), 0).y),
        );
        let p12 = textureLoad(curve_data, curve_loc, 0) - vec4f(render_coord.x, render_coord.y, render_coord.x, render_coord.y);
        let p3 = textureLoad(curve_data, vec2i(curve_loc.x + 1, curve_loc.y), 0).xy - render_coord;
    ...
    for (var curve_index: i32 = 0; curve_index < i32(vband_data.x); curve_index += 1) {
        let curve_loc_val = textureLoad(band_data, vec2i(vband_loc.x + curve_index, vband_loc.y), 0);
        let curve_loc = vec2i(i32(curve_loc_val.x), i32(curve_loc_val.y));
        let p12 = textureLoad(curve_data, curve_loc, 0) - vec4f(render_coord.x, render_coord.y, render_coord.x, render_coord.y);
        let p3 = textureLoad(curve_data, vec2i(curve_loc.x + 1, curve_loc.y), 0).xy - render_coord;
...
// Set true to bypass Slug coverage and show solid rectangles (debug: verify geometry)
const kForceFullCoverage = false;

@fragment
fn main(input: FragmentInput) -> @location(0) vec4f {
    var coverage = 1.0;
    if (!kForceFullCoverage) {
        coverage = slug_render(
            curve_texture,
            band_texture,
            input.texcoord,
            input.banding,
